...
#else /* MAEMO_CHANGES */
static gchar *
get_global_context_id (GtkIMMulticontext *multicontext)
{
  ...
  if (!succeeded)
    {
      ...
      context_id = g_strdup (_gtk_im_module_get_default_context_id (multicontext->priv->client_window));
    }
...
#ifdef MAEMO_CHANGES
      gchar *global_context_id = get_global_context_id (multicontext);
#else /* !MAEMO_CHANGES */

  if (!global_context_id)
    global_context_id = g_strdup (_gtk_im_module_get_default_context_id (multicontext->priv->client_window));
#endif /* MAEMO_CHANGES */
